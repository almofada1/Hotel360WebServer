...
using Dapper;
using Hotel360WebServer.Components.Pages.Views;
...
namespace Hotel360WebServer.Controller
{
    ...
    {
        public static async Task<List<Wgcpaises>> GetPaises(DataContext dbService)
        {
            try
            {
                string sql = @"SELECT codigo, nome FROM wgcpaises ORDER BY codigo ASC";
                var result = await dbService.QueryAsync<Wgcpaises>(sql);
        ...
        public static async Task<bool> UpdateTerceiro(DataContext dbService, Terceiro t)
        {
            try
            {
                ...
                int rowsAffected = await dbService.ExecuteAsync(sql, parameters);
        ...
        public static async Task<Terceiro> GetTerceiroAsync(DataContext dbService,string codigo)
        {
            try
            {
                ...
                var result = await dbService.QueryAsync<Terceiro>(sql, new { Codigo = codigo });
        ...
        public static async Task<List<CheckIn>> CarregaCheckinsDeDataAsync(DataContext dbService)
        {
            try
            {
                string sql = @"
            ...
            ORDER BY R.Quarto, R.Codigo, R.LinhaReserva, RA.DataInicio";
                ...
                var checkInsResult = await dbService.QueryAsync<CheckIn>(sql);
        ...
        public static async Task<DateTime> GetDia(DataContext dbService)
        {
            string sql = "SELECT datahotel FROM whotparametros";
            var result = await dbService.QueryAsync<DateTime>(sql);
            return result.FirstOrDefault();
        }

        public static async Task<Ocupado> VerificaOcupadosQuartoAsync(DataContext dbService, CheckIn checkIn)
        {
            try
            {
                string sql = @"
                ...
                var result = await dbService.QueryAsync<Ocupado>(sql, parameters);
        ...
        public static async Task<bool> EfetuarCheckinAsync(DataContext dbService, CheckIn cki)
        {
            try
            {
                string sql = @"
